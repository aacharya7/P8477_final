...
## parameters for the COVID outbreak 2019 for risk structured model 
#intervention
beta.IDDi = beta.IDUi = 0.03 #transmission from documented 
beta.IUUi = beta.IUDi = 0.1 #transmission in community from undocumented
#no intervention
beta.IDD = beta.IDU = 0.03 #transmission from documented 
beta.IUU = beta.IUD = 0.4 #transmission in community from undocumented
...
risk.parms_noint= c(alpha=alpha,
                   beta.IDD=beta.IDD,
                   beta1=beta1,
                   beta.IUU = beta.IUU,
                   beta.IUD = beta.IUD,
...
times_noint =seq(0,4*7,by=1)
COVID_risk.sim_noint=ode(y=state_risk,times=times_noint,func=COVID_risk,parms=risk.parms_noint)
#run for 8 weeks and after with social distancing
times_int=seq(4*7,365*100,by=1)
state2=c(SD=tail(COVID_risk.sim_noint[,'SD'],1),SU=tail(COVID_risk.sim_noint[,'SU'],1),
         ED=tail(COVID_risk.sim_noint[,'ED'],1),EU=tail(COVID_risk.sim_noint[,'EU'],1),
         ID=tail(COVID_risk.sim_noint[,'ID'],1),IU=tail(COVID_risk.sim_noint[,'IU'],1),
         HD=tail(COVID_risk.sim_noint[,'HD'],1),
         RD=tail(COVID_risk.sim_noint[,'RD'],1), RU=tail(COVID_risk.sim_noint[,'RU'],1),
         DeadD=tail(COVID_risk.sim_noint[,'DeadD'],1),DeadU=tail(COVID_risk.sim_noint[,'DeadU'],1),
         cumInciD=tail(COVID_risk.sim_noint[,'cumInciD'],1),cumInciU=tail(COVID_risk.sim_noint[,'cumInciU'],1))

COVID_risk.sim_int=ode(y=state2,times=times_int,func=COVID_risk,parms=risk.parms_int)
COVID_risk.sim=rbind(COVID_risk.sim_noint,COVID_risk.sim_int[-1,])
